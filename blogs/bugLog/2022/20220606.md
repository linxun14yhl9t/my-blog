...
title: Vue 数据更新了但页面没有更新的 7 种情况汇总及延伸
...
![](https://www.niceleo.cn/blogsImg/15.jpg)
...
> 原因：官方 - 由于 JavaScript 的限制，Vue **不能检测**数组和对象的变化；尤雨溪 - 性能代价和获得用户体验不成正比。(Object.defineProperty() 可以监测数据的变化)
...
1. 给 `<router-view></router-view>` 绑定 `key` 属性，这样 Vue 就会认为这是不同的 `<router-view>`。
